...
export interface MatchResult {
  ...
  maxScore: number;
  verdict: string;
  kootas: Record<string, KootaScore>;
}

// 1. Varna Classification Helper
function getVarna(signNumber: number): { name: string; rank: number } {
  // Brahmin (Water: 4, 8, 12)
  if ([4, 8, 12].includes(signNumber)) return { name: 'Brahmin (Spiritual)', rank: 4 };
  // Kshatriya (Fire: 1, 5, 9)
  if ([1, 5, 9].includes(signNumber)) return { name: 'Kshatriya (Ruling)', rank: 3 };
  // Vaishya (Air: 3, 7, 11)
  if ([3, 7, 11].includes(signNumber)) return { name: 'Vaishya (Commerce)', rank: 2 };
  // Shudra (Earth: 2, 6, 10)
  return { name: 'Shudra (Service)', rank: 1 };
}

// 2. Vashya Classification Helper
function getVashyaGroup(signNumber: number): string {
  ...
  // Vanachar (Wild)
  if (signNumber === 5) return 'Vanachar';
  // Keeta (Insect)
  if (signNumber === 8) return 'Keeta';
  // Manav (Human)
  return 'Manav';
}

// 3. Nakshatra Gana Definitions
const GANA_MAP: Record<number, 'Deva' | 'Manushya' | 'Rakshasa'> = {
...
const NADI_MAP: Record<number, 'Adi' | 'Madhya' | 'Antya'> = {
...
const YONI_MAP: Record<number, string> = {
...
function getFriendshipScore(rulerA: string, rulerB: string): number {
  const idxA = RULER_NAMES.indexOf(rulerA);
  const idxB = RULER_NAMES.indexOf(rulerB);
  if (idxA === -1 || idxB === -1) return 3; // default neutral
...
export function calculateAshtakootMatch(
  moonLongA: number, // Bride's Moon Longitude
  moonLongB: number  // Groom's Moon Longitude
): MatchResult {
  const pA = getPlacementInfo(moonLongA); // Bride (Profile A)
  const pB = getPlacementInfo(moonLongB); // Groom (Profile B)

  const kootas: Record<string, KootaScore> = {};
  let totalScore = 0;
