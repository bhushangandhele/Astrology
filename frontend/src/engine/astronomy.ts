// Zodiac Signs (Rashis) Definitions
...
export interface Nakshatra {
  number: number; // 1-27
  name: string;
  ruler: string; // Lord of the Nakshatra for Vimshottari Dashas
  startDegree: number;
  endDegree: number;
}
 ...
 */
export function getPlacementInfo(longitude: number): PlacementInfo {
  ...
  const rashi = RASHIS[signNumber - 1];
  ...
  const nakshatraObj = NAKSHATRAS[nakIndex];
